...
def trainBarrier(latent_sde, Barrier, batch_size=1024, device='cuda', Test=False):
    ...
    con_opt = torch.optim.Adam(latent_sde.c_net.parameters())
    ...
    for it in range(100):
        ...
        y = 1.5 + 0.5*torch.rand(size=(batch_size, 1))
        ...
        # sortedlosses, indices = torch.sort(torch.Tensor([Lie, Unsafe, Init]))
        # loss = sortedlosses[0]*weight[0] + sortedlosses[1]*weight[1] + sortedlosses[2]*weight[2] + R
        # loss = Lie + Unsafe + Init + R
        loss = Lie_max + (1 - Unsafe_min) / 2 + Init_max + R
        loss.backward()
        optimizer.step()
        con_opt.step()
...
def main(
    ...
    # print(latent_sde.c_net.lin.weight.requires_grad)
    # img_path = os.path.join(train_dir, 'test2.pdf')
    # vis(xs, ts, latent_sde, bm_vis, img_path)
    # assert False
    # print(latent_sde.c_net.lin.weight.data)
    # latent_sde.c_net.lin.weight.data = torch.Tensor([[-0.16,  -2.6]]).to('cuda')
    # latent_sde.fixu = False
    # trainBarrier(latent_sde, Test=True)
    # assert False
    
    ## fine-tune controller
    Barrier = BarrierNN(state_size=2, hidden_size=64).to(device)
    Barrier.load_state_dict(torch.load('./train/Barrier_418.pth'))

    for _ in range(20):
        print(latent_sde.c_net.lin.weight.data)
        latent_sde.fixu = False
        trainBarrier(latent_sde, Barrier)
