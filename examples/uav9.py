...
def make_dataset(batch_size, control, device, N=70):
    # --------------------------- load environment ----------------------------------------
...
def trainBarrier(latent_sde, Barrier, batch_size=128, device='cuda', Test=False, Conly=True):
    ...
    for it in range(_BARRIER_ITERATIONS):
        ...
        loss = 2*Lie_max + 2*(1 - Unsafe_min) + 2*Init_max + 10*R
        # loss = R
        loss.backward()
        if (it+1) % 10 == 0:
...
def main(
        ...
        noise_std=0.01,
        adjoint=False,
        train_dir='./train/uav/0623_warmstart9/',
        method="euler",
):
    ...
    checkpoint = torch.load('./train/uav/warmstart_nominal/nominal_1000model.pth')
    ...
    for ep in range(_TOTAL_ITERATIONS):
        ...
        if ep % 3 == 1:
            optimizer = optim.Adam(latent_sde.parameters(), lr=1e-4)
        else:
