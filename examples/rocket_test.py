...
def metric(xs):
    ...
    for i in range(len(xs)):
        ...
        for j in range(_STEPS):
            ...
            reward += 0.3* LA.norm(xs[i][j][:3] - np.array([_UNSAFE_CENTER[0], _UNSAFE_CENTER[1], 1.5]))
            if i <= 1 and j >= _STEPS - 2:
                print(xs[i][j][:3], reward)
...
def compare(xs, _xs):
    _xs = np.swapaxes(_xs, 0, 1)
    diff = np.array([0]*xs.shape[2])
...
def make_dataset(batch_size, control, device, latent_sde):
    # --------------------------- load environment ----------------------------------------
    ...
    Barrier.load_state_dict(torch.load('./train/rocket/barrier/barrier96_good.pth'))
    barrier_set = []
    ...
    for bs in range(batch_size):
        ...
        for i in range(_STEPS):
            ...
            action.append(curr_u)
            # print(state[:6], curr_u)
            ...
            state = env_dyn(state, curr_u).full().flatten()
        tra = np.array(tra)
        demos.append(tra)
    ...
    ax00.plot(list(range(len(Lie_final))),Lie_final, label='Lie')
    ax00.plot(list(range(len(Unsafe_final))), Unsafe_final, label='$\min_{x_{u} \in X_u} B(x_{u})$')
    ax00.plot(list(range(len(Init_final))), Init_final, label='$\max_{x_{0} \in X_0} B(x_{0})$')
    ...
    bm_vis = torchsde.BrownianInterval(
        t0=0, t1=_STEPS*_DT, size=(batch_size, 20,), device=device, levy_area_approximation="space-time")
    ts = torch.linspace(0, _STEPS*_DT, steps=_STEPS, device=device)
    _xs = latent_sde.sample(batch_size=batch_size, ts=ts, bm=bm_vis).detach().cpu().numpy()
    # compare(demos, _xs)
    _gene_barrier = Barrier(torch.from_numpy(_xs).to(device).float()).squeeze().detach().cpu().numpy()
    print('Max barrier value: ', np.amax(_gene_barrier))
    ...
    # plt.savefig('rocket_barrier.pdf', bbox_inches='tight')
    # assert False 


    print('here', tra.shape, action.shape, _xs[:, 0, :].shape, latent_sde.c_net(torch.from_numpy(_xs[:, 0, :]).to(device).float()).detach().cpu().numpy().shape)
    # assert False

    state_traj_ref = _xs[:, 0, :]
    control_traj_ref = latent_sde.c_net(torch.from_numpy(state_traj_ref).to(device).float()).detach().cpu().numpy()

    rocket.play_animation(rocket_len=2, state_traj=tra, control_traj=action, state_traj_ref=state_traj_ref, control_traj_ref=control_traj_ref,
                       save_option=1, dt=0.1,
...
def trainBarrier(latent_sde, Barrier, batch_size=128, device='cuda', Test=False, Conly=True):
    # if Test:
    Barrier.load_state_dict(torch.load('./train/rocket/barrier/barrier149.pth'))
    #     print('here')
    ...
    for it in range(_BARRIER_ITERATIONS):
        ...
        for _ in range(batch_size):
            ...
            r_I = [2.2*ur / norm * _UNSAFE_RADIUS + _UNSAFE_CENTER[0], 2*vr / norm * _UNSAFE_RADIUS  + _UNSAFE_CENTER[1], np.random.uniform(-_UNSAFE_HEIGHT-1.2, _UNSAFE_HEIGHT + 3 + 1.2)] 
            u = np.random.normal(0, 1)
            v = np.random.normal(0, 1)
        ...
        # loss = Lie_max + (1 - Unsafe_min) + Init_max 
        # loss = Lie / 2.5 + weight[149-it]*((1 - Unsafe_min) + Init_max)
        loss = Lie / 2.5 + 0.2*((1 - Unsafe_min) + Init_max)
        loss.backward()
        if (it+1) % 1 == 0:
            ...
            torch.save(Barrier.state_dict(), './train/rocket/barrier/barrier' + str(it) + '.pth')
        optimizer.step()
        # con_opt.step()
    plt.figure(2)
    ...
    # np.save('Lie_next.npy', np.array(Lie_list))
    # np.save('Unsafe_next.npy', np.array(Unsafe_min_list))
    # np.save('Init_next.npy', np.array(Init_max_list))
...
def main(
    ...
    checkpoint = torch.load('./train/rocket/nominal/training/model_rocket_v1_step_4600.pth')
    ...
    latent_sde.c_net = checkpoint['c_net']
    make_dataset(batch_size=batch_size, control=latent_sde.c_net, device=device, latent_sde=latent_sde)
