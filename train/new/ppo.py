...
class TwoDSys(object):

    def __init__(self, b = (0, 0.2)):
        super(TwoDSys, self).__init__()
        self.b = b
        self.delta = 0.06
    ...
    def reset(self):
        ...
        length = np.sqrt(0.01*np.random.uniform(size=1)[0])
    ...
    def step(self, u):
        x1, x2 = self.state[0], self.state[1]
        disturbance = np.random.normal(0, np.sqrt(self.delta))
        x1_new = x1 + self.delta*x2 + self.b[0]*disturbance
        disturbance = np.random.normal(0, np.sqrt(self.delta))
        x2_new = x2 + self.delta*(u - 0.5*x1**3) + self.b[1]*disturbance
        self.state = np.array([x1_new, x2_new])
        self.t += 1
...
class PPO(object):
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            _xu = torch.from_numpy(np.array([-0.5, 1.45])).float().to('cuda')
            # print(_xu.device, s.device)

            self.penalty = -torch.mean(torch.norm(s - _xu, dim=1))  + torch.Tensor([0.7]).to('cuda') 
...
if __name__ == '__main__':
    ...
    for te in range(TEST_EP):
        ...
        for i in range(EP_LEN):
            ...
            if -1 <= next_state[0] and next_state[0] <= 0 and 1.2 <= next_state[1] and next_state[1] <= 1.7:
                unsafe += 1
                print(te)
                # break
            ep_state.append(state)
            action_collection.append(action)
            reward = -LA.norm(next_state)
            reward += 0.3*LA.norm(next_state - np.array([-0.5, 1.5]))
            ep_reward += reward
            state = next_state
    ...
    state_collection = np.reshape(state_collection, (-1, 100, 2))
    print(state_collection.shape)
    # plt.plot(state_collection[:, 0], state_collection[:, 1])
    # np.save('2D_ppo_tra.npy', state_collection)
    # plt.savefig('ppo.png')
    # np.save('uav_state.npy', np.array(state_collection))
    # np.save('action.npy', np.array(action_collection))
