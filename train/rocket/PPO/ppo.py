...
EP_LEN = 25  # total number of steps for each episode
...
class PPO(object):
    ...
    def choose_action(self, s, greedy=False):
        ...
        if greedy:
            ...
            a[0] = self.actor.action_range * a[0]
        else:
            pi = torch.distributions.Normal(mean, std)
...
def train():
    ...
    for ep in range(EP_MAX):
        ...
        r_I = [ur / norm * _UNSAFE_RADIUS + _UNSAFE_CENTER[0], vr / norm * _UNSAFE_RADIUS  + _UNSAFE_CENTER[1], 1.5] 
        ...
        ini_w = [0.0, -0.0, 0.0]
        ini_state = r_I + ini_v_I + ini_q + ini_w
        s = np.array(ini_state)

        ep_r = 0
        t0 = time.time()
        for t in range(EP_LEN):
            a = ppo.choose_action(s)
            s_ = env_dyn(s, a).full().flatten()
...
if __name__ == '__main__':
    ...
    ppo = PPO(13, 3, method = METHOD)
    ppo.load_model(599900)
    mean_epoch_reward = []
    state_collection = []
    action_collection = []
    idx = 0
    for te in range(TEST_EP):
        ...
        for i in range(EP_LEN):
            ...
            if (a - _UNSAFE_CENTER[0])**2 + (b - _UNSAFE_CENTER[1])**2 <= 1 and -_UNSAFE_HEIGHT <= c and c<= _UNSAFE_HEIGHT+2.4:
                unsafe_count += 1
                idx = te
                # break

            action = ppo.choose_action(state, True)
            next_state = env_dyn(state, action).full().flatten()
            # print(state[: 6], action)
            state_collection.append(state)
            action_collection.append(action)
            reward = -LA.norm(state[:3])
            reward += 0.3* LA.norm(state[:3] - np.array([_UNSAFE_CENTER[0], _UNSAFE_CENTER[1], 1.5]))
            ep_reward += reward
            state = next_state
        mean_epoch_reward.append(ep_reward)
    # print(mean_epoch_reward)       
    print(np.mean(mean_epoch_reward), np.std(mean_epoch_reward))
    print(unsafe_count)
    state_collection = np.array(state_collection)
    state_collection = np.reshape(state_collection, (500, EP_LEN, 13))
    action_collection = np.array(action_collection)
    action_collection = np.reshape(action_collection, (500, EP_LEN, 3))
    np.save('rocket_ppol_tra.npy', np.array(state_collection[idx]))
    np.save('rocket_ppol_act.npy', np.array(action_collection[idx]))
