...
EP_LEN = 75  # total number of steps for each episode
...
class PPO(object):
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            _xu = torch.from_numpy(np.array(ini_state)).float().to('cuda')
            # print(_xu.device, s.device)

            self.penalty = -torch.norm(s - _xu) + torch.Tensor([2]).to('cuda') 
...
if __name__ == '__main__':
    ...
    for te in range(TEST_EP):
        ...
        for i in range(EP_LEN):
            ...
            if (a - _UNSAFE_CENTER[0])**2 + (b - _UNSAFE_CENTER[1])**2 <= 1 and -2 <= c and c <= 5:
                unsafe += 1
                break

            state_collection.append(state)
            action_collection.append(action)
            reward = -LA.norm(next_state[:3])
            # reward += 0.3*LA.norm(next_state[:3] - np.array([_UNSAFE_CENTER[0], _UNSAFE_CENTER[1], 1]))
